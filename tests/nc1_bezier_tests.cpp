...
TEST(NURBS_Chapter1, Bezier2DEndDerivatives) {
  std::vector<glm::dvec2> control_points = {{0, 0}, {1, 1}, {2, 1}, {3, 0}};
  const BezierCurve2D bezier(control_points);

  const glm::dvec2 start_d = bezier.Derivative(0.0);
  const glm::dvec2 end_d = bezier.Derivative(1.0);

  const double n = static_cast<double>(control_points.size() - 1);
  // The start derivative should be n(P1 - P0)
  const glm::dvec2 start_calc = n * (control_points[1] - control_points[0]);
  EXPECT_DOUBLE_EQ(start_calc.x, start_d.x);
  EXPECT_DOUBLE_EQ(start_calc.y, start_d.y);
  // The end derivative should be n(Pn - Pn-1)
  const glm::dvec2 end_calc = n * (control_points[3] - control_points[2]);
...
TEST(NURBS_Chapter1, Bezier3DEndDerivatives) {
  ...
  const glm::dvec3 start_d = bezier.Derivative(0.0);
  const glm::dvec3 end_d = bezier.Derivative(1.0);

  const double n = static_cast<double>(control_points.size() - 1);
  // The start derivative should be n(P1 - P0)
  const glm::dvec3 start_calc = n * (control_points[1] - control_points[0]);
  EXPECT_DOUBLE_EQ(start_calc.x, start_d.x);
  EXPECT_DOUBLE_EQ(start_calc.y, start_d.y);
  // The end derivative should be n(Pn - Pn-1)
  const glm::dvec3 end_calc = n * (control_points[3] - control_points[2]);
  EXPECT_DOUBLE_EQ(end_calc.x, end_d.x);
  EXPECT_DOUBLE_EQ(end_calc.y, end_d.y);
}
