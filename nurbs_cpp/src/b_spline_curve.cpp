...
namespace nurbs {
    ...
    std::vector<glm::dvec2> BSplineCurve2D::Derivatives(double u, uint32_t max_derivative) const {
        if (u < interval_.x) { u = interval_.x; }
        if (u > interval_.y) { u = interval_.y; }
        uint32_t max_deriv = std::min(degree_, max_derivative);
        std::vector<glm::dvec2> derivs(max_deriv + 1, { 0, 0 });
        uint32_t span = static_cast<uint32_t>(knots::FindSpan(degree_, u, knots_));
        std::vector< std::vector<double>> bases = knots::DersBasisFuns(span, u, degree_, static_cast<uint32_t>(control_points_.size() - 1), knots_);
    ...
    std::vector<std::vector<glm::dvec2>> BSplineCurve2D::DerivativeControlPoints(uint32_t max_deriv, uint32_t start, size_t end) const {
        if (start > control_points_.size() || control_points_.empty()) { return {}; }
        ...
        for (uint32_t k = 1; k <= max_deriv; ++k) {
            ...
            for (uint32_t i = 0; i <= total_points - k; ++i) {
                points[k][i] = temp * (points[k - 1][i + 1] - points[k - 1][i])
                    / static_cast<double>(knots_[start + i + degree_ + 1] - knots_[start + i + k]);
            }
        }
        return points;
    }
    ...
    // This does not work and likely isn't implemented correctly
    std::vector<glm::dvec2> BSplineCurve2D::Derivatives2(double u, uint32_t max_derivative) const {
        if (u < interval_.x) { u = interval_.x; }
        if (u > interval_.y) { u = interval_.y; }
        uint32_t max_deriv = std::min(degree_, max_derivative);
        std::vector<glm::dvec2> derivs(max_deriv + 1, { 0, 0 });
        uint32_t span = static_cast<uint32_t>(knots::FindSpan(degree_, u, knots_));
        std::vector <std::vector<double>> bases = knots::AllBasisFuns(span, u, degree_, knots_);
        std::vector<std::vector<glm::dvec2>> points = DerivativeControlPoints(max_deriv, span - degree_, span);
        for (uint32_t k = 0; k <= max_deriv; ++k) {
    ...
    // Chapter 3, ALGORITHM A3.2: CurveDerivsAlgl p93
    std::vector<glm::dvec3> BSplineCurve3D::Derivatives(double u, uint32_t max_derivative) const {
        if (u < interval_.x) { u = interval_.x; }
        if (u > interval_.y) { u = interval_.y; }
        uint32_t max_deriv = std::min(degree_, max_derivative);
