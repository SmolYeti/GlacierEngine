...
namespace vulkeng {
    ...
    void VulkanPipeline::DefaultPipelineConfigInfo(PipelineConfigInfo& config_info) {
        ...
        config_info.input_assembly_info.topology = VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST; // Topology type
        config_info.input_assembly_info.primitiveRestartEnable = VK_FALSE; // Used to break up lists of things
    ...
    void VulkanPipeline::CreateGraphicsPipeline(const std::string& vertex_filepath,
        const std::string& fragment_filepath,
        const PipelineConfigInfo& config_info) {

        assert(config_info.pipeline_layout != VK_NULL_HANDLE && "Cannot create Graphics Pipeline: no pipeline layout provided in the config");
        assert(config_info.render_pass != VK_NULL_HANDLE && "Cannot create Graphics Pipeline: no render pass provided in the config");
        ...
        if (vkCreateGraphicsPipelines(device_->device(), VK_NULL_HANDLE, 1, &pipeline_info,
    ...
    void VulkanPipeline::CreateShaderModule(const std::vector<char>& code, VkShaderModule* shader_module) {
        ...
        if (vkCreateShaderModule(device_->device(), &create_info, nullptr, shader_module) !=
            VK_SUCCESS) {
