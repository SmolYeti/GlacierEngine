...
namespace vulkeng {
    ...
    void PointLightSystem::CreatePipeline(VkRenderPass render_pass) {
        ...
        VulkanPipeline::EnableAlphaBlending(pipeline_config);
    ...
    void PointLightSystem::Render(const FrameInfo& frame_info) {
        std::map<float, VulkanGameObject::id_t> sorted_lights;
        ...
        for (auto& key_value : frame_info.game_objects) {
            ...
            auto offset = frame_info.camera.position() - obj.transform_.translation;
            float distSqrd = glm::dot(offset, offset);
            sorted_lights[distSqrd] = obj.id();
        }
